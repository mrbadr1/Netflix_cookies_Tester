...
from config import *
from UI.ui_initializer import init_ui
#-----------------------------------------------------------------
class CookieChecker(QThread):
    update_progress = pyqtSignal(int)
    update_result = pyqtSignal(int, int)
    def __init__(self, cookies_list, test_cookie_function):
        super().__init__()
        self.cookies_list = cookies_list
        self.test_cookie = test_cookie_function

    def run(self):
        total_cookies = len(self.cookies_list)
        working_cookies = 0
        dead_cookies = 0

        for index, cookie in enumerate(self.cookies_list):
            # Perform the cookie checking logic here
            ...
            # Update the progress
            progress = int((index + 1) / total_cookies * 100)
            self.update_progress.emit(progress)

        self.update_result.emit(working_cookies, dead_cookies)
#-----------------------------------------------------------------
class CookieTester(QWidget):
    def __init__(self):
        ...
        self.file_names = []
        self.init_ui = init_ui.__get__(self)
    
        # Create a QTimer
        self.timer = QTimer()
        self.timer.timeout.connect(self.refresh_app)
        self.timer.start(1)  # Refresh the app every 10 milliseconds
